...
fn main() -> Result<()> {
    ...
    let lazy_file = cache.get_lazy("expensive_computation.txt", |mut file| {
    ...
    println!("File exists on disk: {}", lazy_file.path().exists());

    if lazy_file.path().exists() {
    ...
    lazy_file.open()?.read_to_string(&mut content1)?;

    println!("File now exists on disk: {}", lazy_file.path().exists());
    ...
    lazy_file.open()?.read_to_string(&mut content2)?;
