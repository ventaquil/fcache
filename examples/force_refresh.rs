...
fn main() -> anyhow::Result<()> {
    ...
    let file = cache.get("dynamic_data.txt", move |mut file| {
    ...
    file.open()?.read_to_string(&mut content1)?;
    ...
    file.open()?.read_to_string(&mut content2)?;
    ...
    file.open()?.read_to_string(&mut content3)?;
